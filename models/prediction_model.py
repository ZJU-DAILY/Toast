...
class Encoder(nn.Module):
    ...
    def forward(self, data: torch.Tensor, features, edge_index):
        """
...
class Decoder(nn.Module):
    ...
    def forward(self, hidden_embeds, features, labels, edge_index, train_mode=True):
        ...
        for idx in range(length):
            ...
            inital_hidden = hidden_embeds[0].unsqueeze(dim=0).reshape(1, num_nodes * batch_size, -1)
            outputs, hiddens = self.rnn_layer(data, inital_hidden)
            outputs = outputs.reshape((num_nodes, batch_size, 1, -1))
            hidden_embeds[0] = hiddens.squeeze().reshape(num_nodes, batch_size, -1)

            outputs = self.meta_gat(outputs, edge_index, features)

            inital_hidden = hidden_embeds[1].unsqueeze(dim=0).reshape(1, num_nodes * batch_size, -1)
            outputs = outputs.reshape((num_nodes * batch_size, 1, -1))
            outputs, hiddens = self.meta_rnn(outputs, inital_hidden)
            outputs = outputs.reshape((num_nodes, batch_size, 1, -1))
